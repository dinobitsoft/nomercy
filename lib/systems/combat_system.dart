...
class CombatSystem {
  ...
  void processAttack({
    ...
    final isBlocked = target.isBlocking && _canBlock(target, attacker);
    if (isBlocked) {
      damage *= 0.3; // 70% damage reduction
      _totalBlocks++;

      // Drain target stamina
      target.stamina -= GameConfig.blockStaminaDrain;
      if (target.stamina < 0) {
        target.stamina = 0;
  ...
  void applyDamage({
    ...
    final oldHealth = target.health;
    target.takeDamage(damage);
    final newHealth = target.health;
  ...
  bool _canBlock(GameCharacter blocker, GameCharacter attacker) {
    ...
    return facingSameDirection && blocker.stamina >= GameConfig.blockStaminaDrain;
  }
