...
class Projectile extends PositionComponent with HasGameReference<ActionGame> {
  ...
  void onLoad() async {
    ...
    priority = 75; // Between platforms (10) and character (90-100)
  ...
  void update(double dt) {
    ...
    if (enemyOwner != null) {
      if (position.distanceTo(game.character.position) < 30) {
        game.character.takeDamage(damage);
        ...
        EventBus().emit(CharacterDamagedEvent(
          ...
          damage: damage, 
          remainingHealth: game.character.characterState.health,
          healthPercent: (game.character.characterState.health / 100) * 100, // Assuming 100 is max
    ...
    if (owner != null) {
      ...
      for (final enemy in game.enemies) {
        if (position.distanceTo(enemy.position) < 30) {
          ...
          EventBus().emit(CharacterDamagedEvent(
            ...
            damage: damage,
            remainingHealth: enemy.characterState.health,
            healthPercent: (enemy.characterState.health / 100) * 100,
            damageSource: 'projectile'
          ));
      ...
      if (game.enableMultiplayer) {
        for (final entry in NetworkManager().remotePlayers.entries) {
          ...
          if (remotePlayer.characterState.health > 0 && position.distanceTo(remotePlayer.position) < 30) {
            ...
            EventBus().emit(CharacterDamagedEvent(
              ...
              damage: damage,
              remainingHealth: remotePlayer.characterState.health,
              healthPercent: (remotePlayer.characterState.health / 100) * 100,
              damageSource: 'projectile'
            ));
