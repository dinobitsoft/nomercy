  ...
  void connect(String characterClass, CharacterStats stats, ActionGame gameInstance, {String? roomId}) {
    ...
    socket.on('player-attacked', (data) {
      ...
      if (remotePlayers.containsKey(playerId)) {
        ...
        remotePlayer.characterState.isAttacking = true;
        remotePlayer.characterState.attackAnimationTimer = 0.2;
    ...
    socket.on('player-died', (data) {
      ...
      if (deadId == socket.id) {
        ...
        game?.character.characterState.health = 0;
      } else if (remotePlayers.containsKey(deadId)) {
        // Remote player died
        remotePlayers[deadId]!.characterState.health = 0;

        // Award money to killer if it's local player
        if (killerId == socket.id) {
          game?.character.stats.money += 20;
    ...
    socket.on('player-respawned', (data) {
      ...
      if (respawnedId == socket.id) {
        ...
        game?.character.characterState.health = 100;
        game?.character.position = Vector2(
          data['position']['x'].toDouble(),
          data['position']['y'].toDouble(),
        );
        ...
        remotePlayer.characterState.health = 100;
    ...
    socket.on('stat-upgraded', (data) {
      ...
      game?.character.stats.money = data['money'];
    });
  ...
  void update(double dt) {
    ...
    if (networkUpdateTimer >= 0.1) {
      sendPosition(
        ...
        game!.character.facingRight,
        game!.character.velocity.x,
        game!.character.velocity.y,
      );
