...
class GameManager extends Component with HasGameReference<ActionGame> {
  ...
  void spawnMiniBoss() {
    ...
    miniBoss.characterState.health = 200;
  ...
  void spawnBoss() {
    ...
    boss.characterState.health = 300;
    boss.stats.attackDamage *= 1.5;
    boss.characterState.maxStamina = 200;
    boss.characterState.stamina = 200;
    ...
    print('BOSS HEALTH: ${boss.characterState.health}');
  ...
  void _defeatBoss() {
    ...
    // Massive rewards
    game.character.stats.money += 500;
    game.character.characterState.health = 100; // Full heal
  ...
  void _upgradeAllBots() {
    for (final enemy in game.enemies) {
      enemy.characterState.health = (enemy.characterState.health * 1.1).clamp(0, 150);
  ...
  void _scaleEnemyDifficulty(GameCharacter enemy, int wave) {
    ...
    enemy.characterState.health = (100 * healthBonus * difficultyMultiplier).clamp(50, 200);
    enemy.stats.attackDamage *= damageBonus * difficultyMultiplier;

    // Higher waves get more stamina
    if (wave >= 5) {
      enemy.characterState.maxStamina = 120;
      enemy.characterState.stamina = 120;
    }
  }
  ...
  void _rewardPlayer() {
    ...
    game.character.characterState.health = (game.character.characterState.health + 20).clamp(0, 100);
