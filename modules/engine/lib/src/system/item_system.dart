...
class ItemSystem {
  ...
  void _applyHealthPotion(GameCharacter character, HealthPotion potion) {
    final oldHealth = character.characterState.health;
    character.characterState.health = math.min(100, character.characterState.health + potion.healAmount);
    final actualHeal = character.characterState.health - oldHealth;
    ...
    if (actualHeal > 0) {
      ...
      _eventBus.emit(CharacterHealedEvent(
        ...
        newHealth: character.characterState.health,
        healSource: 'potion',
      ));
  ...
  GameCharacter? _findCharacter(String characterId) {
    if (game.character.stats.name == characterId) {
  ...
  void update(double dt) {
    ...
    for (final drop in List.from(_activeDrops)) {
      final distance = drop.position.distanceTo(game.character.position);
